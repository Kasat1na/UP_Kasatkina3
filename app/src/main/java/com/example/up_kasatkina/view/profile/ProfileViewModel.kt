...
class ProfileViewModel: ViewModel() {
    var profiles by mutableStateOf<List<profiles>>(listOf())//хранит список профилей пользователей

    // Функция для загрузки профиля текущего
    fun showProfile() {
    ...
    // для обновления профиля текущего пользователя
        ...
        viewModelScope.launch {
            try {
                ...
                if (user != null) { // проверяем что пользователь вошел
                    ...
                    showProfile() // обновляем показ данные после обновления
                }
